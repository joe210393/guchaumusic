...
// Let's spawn it.
try {
    // Simple check: if DB file doesn't exist or is empty (0 bytes), run init.
    // Actually, init-sqlite.js is now safe to run even if DB exists (it checks for tables).
    // So we can just run it.
    console.log('Running DB initialization check...');
    const child = fork(initScript);
    child.on('exit', (code) => {
        if (code === 0) console.log('DB Init check completed.');
        else console.error('DB Init check failed with code', code);
        startServer();
    });
} catch (e) {
    console.error('Failed to run DB init:', e);
    startServer();
}
...
function startServer() {
    ...
    const pages = ['index.html','about.html','about-guchau.html','about-music.html','about-story.html','about-history.html','service-courses.html','service-commercial.html','service-sales.html','service-space.html','service-tourism.html','product-detail.html','media-records.html','booking.html','blog.html','blog-post.html','news.html','news-post.html','leaderboard.html','leader.html','plans.html','plan.html','contact.html','trial.html','login.html','register.html','user.html','admin/members.html','admin/booking.html'];
    pages.forEach(p => {
